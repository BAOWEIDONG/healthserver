...
| 代理人端 | `agent.html` | 手机号+验证码登录；代理人见：班期管理+我的客户；管理员另见：项目管理+账户管理 |
...
- **账户**管理仅含管理员/代理人账户（手机号+验证码登录），客户不作为账户维护
...
7. **我的预约**：按班期分组的卡片 → 详情页（身份证脱敏、商城购买入口、提交时间）→ 可取消（释放名额，**取消记录保留**，卡片/详情标「已取消」）→ 详情页展示**到场签到**状态（代理人确认后更新并回显签到时间）
...
- **账户管理**（仅管理员）：管理员/代理人账户增删改（姓名/登录手机号/角色）
- **班期名单**：项目统计行点击弹出分项目名单 Sheet（头像/姓名/手机/身份证脱敏）；乘客表末列**到场签到**（代理人点「确认到场」/「撤销」，副标题显示已签到 x/y；签到结果同步到学员端）
...
- 黄金雷针月卡 8 次 · ¥4500（P1.mall.price 已配置）
